...
export default function MiniCartDropdown() {
  const {totalQuantity,miniCartItems,totalPrice} = usePage().props;

  return(
    <div className="dropdown dropdown-end gap-10">
                    <div
                        tabIndex={0}
                        role="button"
                            ...
                            <div className="my-4 max-h-[300px] overflow-auto">
                              ...
                              {miniCartItems.map((item,index) => (
                                <div className="flex gap-4 p-3" key={item.id}>
                                  <Link className="w-16 h-16 flex justify-center items-center" href={productRoute(item)}>
